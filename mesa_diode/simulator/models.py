# -*- coding: utf-8 -*-
"""Реестр моделей тока p-n перехода: CurrentModel и параметры подгонки FitParam.

Модель тока — ток перехода при напряжении на нём V_d, её поля окна и
параметры подгонки; эквивалентная схема (6.1) общая (physics.circuit).
Кто и как пользуется реестром, как добавить модель — методичка, п. 13.6.
"""
...
class FitParam:
    """Параметр автоподгонки (методичка, п. 13.6, таблица FitParam)."""
...
FIT_PARAMS = {
    # n ∈ [1, 2]: границы — методичка, п. 9А.2.
